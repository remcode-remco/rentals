...
const PopupMessage = ({message,setMessage}:{message:Message|null,setMessage:(message:Message|null)=>void}) => {
  ...
  return (
      <div className={`z-50 fixed bottom-0 left-0 right-0 z-40 overflow-y-auto 
        ${classNames}
        `}
      >
        <div className='mx-auto max-w-sm flex justify-center my-10'>
          <span onClick={()=>setMessage(null)} className={`
        ${message && message.error ? "bg-red-500" : "bg-green-500"} text-white py-2 px-3 text-xl rounded shadow-lg shadow-gray-600 flex p-2`}>
